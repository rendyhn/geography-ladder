...
${Fig(planeSvg({ W: 380, H: 230, x: [0, 6500], y: [0, 15], step: [1000, 2.5], tickX: 2000, tickY: 5, fmtX: v => F(v), xl: T`depth (km)`, yl: 'km/s', extra: (X, Y, D = [[0, 5.8, 3.2], [35, 5.8, 3.2], [35, 8.0, 4.4], [220, 8.0, 4.4], [400, 8.9, 4.8], [670, 10.2, 5.6], [1000, 11.1, 6.2], [2000, 12.5, 6.8], [2891, 13.7, 7.3], [2891, 8.0, 0], [4000, 9.4, 0], [5150, 10.3, 0], [5150, 11.0, 3.5], [6371, 11.3, 3.7]]) => sPline(D.map(([d, p]) => [X(d), Y(p)]), 'mf-c1', ' fill="none" stroke-width="2.2"') + sPline(D.map(([d, , v]) => [X(d), Y(v)]), 'mf-c2', ' fill="none" stroke-width="2.2"') + [2891, 5150].map(d => sL(X(d), Y(0), X(d), Y(15), 'mf-grid', ' stroke-dasharray="4 3"')).join('') + sT(X(1500), Y(12.6), T`P waves`, 'mf-small') + sT(X(1500), Y(7.2), T`S waves`, 'mf-small') + sT(X(4000), Y(2.2), T`no S waves in the liquid outer core`, 'mf-small') + sT(X(1450), Y(14.4), T`mantle`, 'mf-small') + sT(X(4020), Y(14.4), T`outer core`, 'mf-small') + sT(X(5760), Y(14.4), T`inner core`, 'mf-small'), label: T`Speeds of P and S earthquake waves with depth; S waves vanish in the liquid outer core` }), T`Earthquake wave speeds with depth (after the PREM Earth model, Dziewonski and Anderson 1981). S waves cannot cross liquid, so their disappearance at $2\,891\,\mathrm{km}$ shows the outer core is molten.`)}
...
${FigW(rockSettingsSvg(), T`Where each rock type forms: lava cooling fast at the surface (extrusive igneous), magma cooling slowly underground (intrusive), layers of sediment in the sea (sedimentary) and rock baked next to hot magma (metamorphic).`)}
...
${FigW(numberLineSvg({ min: 0, max: 8, step: 1, W: 560, H: 116, marks: [{ v: 1, label: T`Kilauea`, cls: 'mf-s2', below: true }, { v: 4, label: T`Merapi 2010` }, { v: 6, label: T`Krakatau 1883`, below: true, cls: 'mf-s4' }, { v: 7, label: T`Tambora 1815`, cls: 'mf-s4' }, { v: 8, label: T`Toba ~74 000 yr ago`, below: true, cls: 'mf-s4' }], label: T`Volcanic Explosivity Index scale from 0 to 8 with Indonesian eruptions marked` }), T`The Volcanic Explosivity Index: each step is about 10 times more erupted material. Indonesia holds some of the largest eruptions known (VEI values from the Smithsonian Global Volcanism Program).`)}
...
${FigW(stepsSvg([T`weathering`, T`mass movement`, T`erosion`, T`transport`, T`deposition`], { W: 640, label: T`The chain of exogenic processes from weathering to deposition` }), T`Exogenic processes work as a chain: rock is broken up where it lies, moved downhill by gravity, carried away by water, wind or ice, and finally laid down.`)}
<h3>Mass movement</h3>
...
${FigW(massMovementSvg(), T`Mass movement from fast to slow: rockfall (free fall from a cliff), landslide (a block sliding on a slip surface), mudflow (wet debris flowing like a liquid) and creep (slow, shown by tilted posts and trees).`)}
