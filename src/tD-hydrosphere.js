...
${Fig(barsSvg([[T`clay`, 45, 'mf-s4'], [T`sand`, 35, 'mf-s1'], [T`gravel`, 25, 'mf-s1'], [T`sandstone`, 15, 'mf-s2'], [T`granite`, 1, 'mf-s3']], { W: 380, H: 220, yMax: 50, step: 10, yl: T`porosity (%)`, label: T`Typical porosity: clay 45 percent, sand 35, gravel 25, sandstone 15, unfractured granite about 1` }), T`Typical porosities. Clay holds the most water but its pores are tiny and poorly connected, so water hardly moves through it: good aquifers need high <i>permeability</i> as well (sand, gravel).`)}
<h3>Problems</h3>
...
${Fig(planeSvg({ W: 340, H: 240, x: [0, 32], y: [-1000, 0], step: [5, 125], tickX: 10, tickY: 250, fmtY: v => (v <= -1000 ? '' : F(-v)), xl: ' ', yl: T`depth (m)`, extra: (X, Y) => sPline([[29, 0], [28.5, -60], [20, -150], [12, -300], [8, -500], [5, -1000]].map(([t, d]) => [X(t), Y(d)]), 'mf-c1', ' fill="none" stroke-width="2.2"') + sR(X(0), Y(-60), X(32) - X(0), Y(-300) - Y(-60), 'mf-f4') + sT(X(22), Y(-190), T`thermocline`, 'mf-lab-b') + sT(X(18), Y(-750), T`cold deep water`, 'mf-small') + sT(X(31), Y(-960), 'T (°C)', 'mf-var', 'end'), label: T`Typical tropical sea temperature profile: warm mixed layer, a thermocline where it drops fast, then cold deep water` }), T`A typical tropical ocean profile: a warm, well-mixed surface layer, a thermocline where the temperature falls quickly, and cold water below about 1 000 m.`)}
