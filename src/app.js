...
const UI_EN = {
  ...
  heroEyebrow: 'Geography · Topic by topic',
  heroTitle: 'From the map to the whole planet, one rung at a time.',
  heroLede: '⟦0⟧ topics for senior high school and university, laid out in tracks from maps and the solid Earth to population, regions and global issues. Each has an illustrated lesson and a practice worksheet that is generated fresh every time you open it, with worked solutions, a fold-out answer key and print-ready pages.',
  ...
  physLadder: 'Physics Ladder',
  dirN: 'N', dirS: 'S', dirE: 'E', dirW: 'W',
  ...
  date: 'Date',
  score: 'Score',
  fillNote: 'Fill-in answers accept whole numbers, decimals and fractions like <code>3/4</code>. You may type the unit too, as in <code>25 km</code> or <code>150 people/km²</code>. Answers within about 1% are accepted unless the question asks for an exact count.',
...
const extTitle = (kind, id) => metaOf(kind, id, 0, EXT[kind].topics[id]);
...
const CREDIT = 'Geography Ladder · @rendyhn 2026';
...
/* a link end is a topic here, a planned topic, or math:<id> / phys:<id> on the sister sites */
const linkEnd = id => { const m = /^(math|phys):(.+)$/.exec(id); return m ? (EXT[m[1]].topics[m[2]] ? { ext: m[1], extId: m[2] } : null) : TOPICS.get(id) || SOON.get(id) || null; };
LADDER.forEach(e => {
  const a = linkEnd(e.from), b = linkEnd(e.to);
  if (!a || !b || a.review || b.review || b.ext) { console.warn('ladder: unknown topic in', e.from, '->', e.to); return; }
  (b.needs || (b.needs = [])).push(e); if (!a.ext) (a.opens || (a.opens = [])).push(e);
});

/* ---------------- settings (per-viewer convenience only) ---------------- */
const store = {
  get(k, d) { try { const v = localStorage.getItem('geoladder:' + k); return v == null ? d : JSON.parse(v); } catch (e) { return d; } },
  set(k, v) { try { localStorage.setItem('geoladder:' + k, JSON.stringify(v)); } catch (e) { /* storage unavailable */ } },
};
...
const PU = '(?:[kmgµμncp]?(?:ohms?|pa|hz|cal|wh|ev|bq|d|u|n|j|w|v|a|c|g|m|s|l|k|t|f)|°c|°f|°|h|min|rad|rev|rpm|atm|hpa|mbar|mb|ha|ppm|people|persons?|jiwa|orang|years?|yrs?|tahun|thn|jam|hari|days?)(?:\\^?\\(?-?[123]\\)?)?';
...
function close(x, v, tol, typed, rtol) {
  ...
  if (rtol != null && tol == null) return Math.abs(x - v) <= 1e-9 * Math.abs(v);   // measured answers: relative tolerance only, so tiny answers are checked properly
  if (tol != null) return Math.abs(x - v) <= tol + 1e-9;
  if (Math.abs(x - v) <= Math.max(1e-6, 1e-6 * Math.abs(v))) return true;
  if (rtol != null) return false;   // measured answers: the stated tolerance only, no coarse rounding
...
/* numeric distractors: plausible slips (a factor of 2, 10, …) rather than answers one digit away */
...
/* ---------------- home ---------------- */
function renderHome() {
  document.title = 'Geography Ladder';
  const first = LEVELS[0].topics[0];
  main.innerHTML = `
  <section class="hero">
    <div class="hero-copy">
      <div class="hero-logo" aria-hidden="true"><svg viewBox="0 0 64 64"><rect x='1' y='1' width='62' height='62' rx='14' fill='#141B2B' stroke='#FFFFFF' stroke-opacity='0.14' stroke-width='2'/><defs><clipPath id='gl-x'><circle cx='32' cy='32' r='21'/></clipPath></defs><circle cx='32' cy='32' r='21' fill='#2C6DB0'/><g clip-path='url(#gl-x)'><path d='M13 17C18 13 25 13 27 17C29 21 24 24 25 29C26 34 22 38 18 36C14 34 16 28 12 25Z' fill='#6CC070'/><path d='M36 22C40 19 47 20 50 25C53 30 50 34 46 33C42 32 43 37 45 41C47 46 42 50 38 47C34 44 37 38 35 34C33 30 33 25 36 22Z' fill='#6CC070'/><g stroke-width='2' stroke-linecap='round'><path d='M8 18.5H56' stroke='#F58BB0'/><path d='M8 27.5H56' stroke='#A9B2FF'/><path d='M8 36.5H56' stroke='#34CDB8'/><path d='M8 45.5H56' stroke='#FB8B52'/></g><ellipse cx='32' cy='32' rx='9.5' ry='21' fill='none' stroke='#FFFFFF' stroke-opacity='0.7' stroke-width='1.6'/></g><circle cx='32' cy='32' r='21' fill='none' stroke='#FFFFFF' stroke-width='3'/></svg></div>
...
function renderTopic(t, tab) {
  ...
  document.title = `${tTitle(t)} · Geography Ladder`;
  main.innerHTML = `
...
function ladderLinks(t) {
  const item = (o, e) => {
    const x = o.ext && EXT[o.ext];
    const head = x
      ? `<a href="${x.url}?lang=${I18N.lang}#${o.extId}" target="_blank" rel="noopener" class="rungs-math"><span class="rung" aria-hidden="true">${x.icon}</span><span>${esc(extTitle(o.ext, o.extId))}</span><small>${esc(ui(x.label))} ↗</small></a>`
      : o.soon
